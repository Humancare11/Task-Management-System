...
import { listProjectActivities } from "../../api/activities.js";

const PAGE_SIZE = 20;

// Contextual activity feed for a single project (GET /api/activities?project_id=).
export default function ProjectActivityTab({ projectId }) {
  ...
  const fetchPage = useCallback(
    async (nextPage) => {
      ...
      try {
        const res = await listProjectActivities(projectId, {
          page: nextPage,
          limit: PAGE_SIZE,
        });
  ...
  // Reset and refetch whenever the project changes.
  useEffect(() => {
    setItems([]);
    setPage(1);
    setTotalPages(1);
  ...
  // Live updates — only activities belonging to this project.
  useActivitySocket({
    projectId,
  ...
  return (
    <div className="px-1 py-2 sm:px-0">
      <ActivityFeed
        ...
        emptyDescription="Actions taken on this project will appear here."
      />
    </div>
  );
}
